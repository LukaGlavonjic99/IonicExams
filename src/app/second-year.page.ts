...
export class SecondYearPage implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    async getData() {
      this.sub = this.dataService.getExams('examsSecondYear').subscribe((res) => {
        this.exams = res;
    ...
    async deleteExam(exam: Exam) {
      if (exam.id) {
        try {
          await this.dataService.deleteExam(exam.id,'examsSecondYear');
          this.getData();  
        } catch (error) {
    ...
    async goToAddPage() {
      const modal = await this.modalCtrl.create({
        ...
        componentProps: { collectionName: 'examsSecondYear' }
      });
