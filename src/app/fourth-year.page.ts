...
@Component({
  ...
  templateUrl: 'fourth-year.page.html',
  styleUrls: ['fourth-year.page.scss'],
})
export class FourthYearPage implements OnInit, OnDestroy {
  ...
  async getData() {
    this.sub = this.dataService.getExams('examsFourthYear').subscribe((res) => { 
      this.exams = res;
    ...
    async deleteExam(exam: Exam) {
      if (exam.id) {
        try {
          await this.dataService.deleteExam(exam.id,'examsFourthYear');
          this.getData();  
        } catch (error) {
    ...
    async goToAddPage() {
      const modal = await this.modalCtrl.create({
        ...
        componentProps: { collectionName: 'examsFourthYear' }
      });
